...
BOOL StackWalker::ShowCallstack(std::string *out, HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
...
#error "Platform not supported!"
  ...
  const int MAX_FRAME_NUM = 6;
  for (frameNum = 0; frameNum < MAX_FRAME_NUM; ++frameNum )
  {
    ...
    if (frameNum == 0)
      et = firstEntry;
    this->OnCallstackEntry(et, csEntry, out);
    
    if (s.AddrReturn.Offset == 0)
    {
      this->OnCallstackEntry(lastEntry, csEntry, out);
